...
class Dom2D{
	public:
		...
		int irst;	// restart ? 1:yes, 0:no
		double tr1,tr2;	// time range (when restarted) 
...
//		PML( Dom2D dom, double a, double b);
		...
		void gridNum(int ityp);
		void damp(char dir_name[128],int ipml,int isum);
		void import(int ipml,int isum);
	private:
};
...
class Fld2D{
	public:
		...
		void damp(char dir_name[128],int isum);
		void import(int isum);
