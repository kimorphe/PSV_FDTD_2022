...
void Fld2D:: damp(char dir_name[128], int isum){
	char fname[128];
	sprintf(fname,"%s/anc%d.dat",dir_name,isum);
	int i,j;
	FILE *fp=fopen(fname,"w");
	fprintf(fp,"Ndiv=%d %d\n",Ndiv[0],Ndiv[1]);
	...
	for(int ipml=0;ipml<npml;ipml++){
		pml[ipml].damp(dir_name,ipml,isum);
	};
};
void Fld2D:: import(int isum){
	char fname[128];
	sprintf(fname,"Rst/anc%d.dat",isum);
	int i,j;
	...
	printf("** Importing PML data from\n");
	for(int ipml=0;ipml<npml;ipml++){
			pml[ipml].import(ipml,isum);
	};
	printf("** Done\n");
};
